...
job_stats=$( { /usr/bin/time -v Rscript seurat_Standard_PCA.R $CELLS $GENES; } 2>&1 )
echo $job_stats
...
echo $CELLS $GENES Standard Seurat $user_time_seconds $system_time_seconds $elapsed_time_seconds $raw_elapsed_time $max_resident_set_size $avg_resident_set_size $memory_used_bytes >> output.txt

# bsub -o OUT.%J.%I.out -q rerunnable -R 'rusage[mem=12000]' -W 24:00 -J sstand[1-20] "bash seurat_Standard_PCA.sh"
