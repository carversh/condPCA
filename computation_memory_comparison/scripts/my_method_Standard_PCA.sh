...
job_stats=$( { /usr/bin/time -v python my_method_Standard_PCA.py $CELLS $GENES; } 2>&1 )
echo $job_stats
...
echo $CELLS $GENES Standard my_method $user_time_seconds $system_time_seconds $elapsed_time_seconds $raw_elapsed_time $max_resident_set_size $avg_resident_set_size $memory_used_bytes >> output.txt

# bsub -o OUT.%J.%I.out -q rerunnable -R 'rusage[mem=12000]' -W 1:00 -J smy[1] "bash my_method_Standard_PCA.sh"
