...
if (is.na(flag_sim_100_cts)){
    ...
    output = readRDS("/data/gusev/USERS/scarver/MORABITO_scratch/RNA/fitted_parameters_Gamma_poiss.RDS")
    ...
    ct_prop = read.table("./ct_proportions_100_cts.txt",header=TRUE, row.names=1)
...
#     print(dim(final_meta))
    ...
    print(glue("transposed data dim: {dim(unscaled_data)}"))
    if (is.na(flag_sim_100_cts)){
        write.table(unscaled_data, file=glue("/PHShome/sv433/Gusev_Lab/Simulation/Gamma-Poisson/cNMF/{method}_{total_cells}_{seed}_{state_type}_{dim}_gene_{sub('[.]', 'p', perc_genes)}_cell_{sub('[.]', 'p', perc_cells)}.tab"), append = FALSE, sep = "\t", dec = ".",row.names = TRUE, col.names = TRUE)
    } else {
    write.table(unscaled_data, file=glue("/PHShome/sv433/Gusev_Lab/Simulation/Gamma-Poisson/cNMF/{method}_{total_cells}_{seed}_{state_type}_{dim}_gene_{sub('[.]', 'p', perc_genes)}_cell_{sub('[.]', 'p', perc_cells)}_flag_{flag_sim_100_cts}.tab"), append = FALSE, sep = "\t", dec = ".",row.names = TRUE, col.names = TRUE)
    }
} else {
